require([
    ...
    var successHandlers = {
        ...
        debug: function (result, $container) {
            ...
            if (Array.isArray(result)) {
                ...
                var lisHtml = result.map(function (err) {
                    return '<li class="magmodules-messagebird-result_debug-item"><strong>' + err.date + '</strong><p>' + err.msg + '</p></li>';
                }).join('');
        ...
        error: function (result, $container) {
            ...
            if (Array.isArray(result)) {
                ...
                var lisHtml = result.map(function (err) {
                    return '<li class="magmodules-messagebird-result_error-item"><strong>' + err.date + '</strong><p>' + err.msg + '</p></li>';
                }).join('');
        ...
        test: function (result, $container) {
            ...
            var lisHtml = result.map(function (test) {
                ...
                var supportLinkHtml = test.support_link ?
                    '<a target="_blank" href="' + test.support_link + '" class="magmodules-messagebird-icon__help-rounded"></a>' : '';
                ...
                return '<li class="magmodules-messagebird-result_test-item ' + test.result_code
        ...
        version: function (result, $container) {
            ...
            if (this.compare(latestVersion, currentVersion) < 0) {
                resultHtml = '<strong class="magmodules-messagebird-version magmodules-messagebird-icon__thumbs-up">'
                    + $.mage.__('Great, you are using the latest version.')
                    + '</strong>';
            } else {

                var translatedResult = $.mage.__('There is a new version available <span>(%1)</span> see <button type="button" id="magmodules-messagebird-button_changelog">changelog</button>.')
                    .replace('%1', latestVersion);

                resultHtml = '<strong class="magmodules-messagebird-version magmodules-messagebird-icon__thumbs-down">'
                    + translatedResult
                    + '</strong>';
            }
        ...
        changelog: function (result, $container) {
            ...
            var lisHtml = Object.keys(result).map(function (key) {
                ...
                return '<li class="magmodules-messagebird-result_changelog-item"><b>'
                    + version + '</b><span class="magmodules-messagebird-divider">|</span><b>'
                    + date + '</b><div>'
                    + resultHtml + '</div></li>';
            }).join('');
    ...
    // init debug modal
    initModal('#magmodules-messagebird-result_debug-modal', {
        type: 'popup',
        responsive: true,
        innerScroll: true,
        ...
        buttons: [
            {
                ...
                class: 'magmodules-messagebird-button__download magmodules-messagebird-icon__download-alt',
                click: function () {

                    var elText = document.getElementById('magmodules-messagebird-result_debug').innerText || '';
    ...
    // init error modal
    initModal('#magmodules-messagebird-result_error-modal', {
        type: 'popup',
        responsive: true,
        innerScroll: true,
        ...
        buttons: [
            {
                ...
                click: function () {
                    ...
                    var elText = document.getElementById('magmodules-messagebird-result_error').innerText || '';
                    var link = document.createElement('a');
    ...
    // init selftest modal
    initModal('#magmodules-messagebird-result_test-modal', {
        type: 'popup',
        responsive: true,
        innerScroll: true,
    ...
    initModal('#magmodules-messagebird-result_changelog-modal', {
        type: 'popup',
        responsive: true,
        innerScroll: true,
        title: 'Changelog',
    ...
    $('.magmodules-messagebird-result_version-wrapper').loader({texts: ''});

    /**
     * Ajax request event
     */
    $(document).on('click', '[id^=magmodules-messagebird-button]', function () {
        var actionName = this.id.split('_')[1];
        var $modal = $('#magmodules-messagebird-result_' + actionName + '-modal');
        var $result = $('#magmodules-messagebird-result_' + actionName);

        if (actionName === 'version') {
            $(this).fadeOut(300).addClass('magmodules-messagebird-disabled');
            $modal = $('.magmodules-messagebird-result_' + actionName + '-wrapper');
            $modal.loader('show');
        } else {
        ...
        new Ajax.Request($modal.data('magmodules-messagebird-endpoind-url'), {
            loaderArea: false,
            asynchronous: true,
